    ...
    case '80':
        ...
        if (mysql_num_rows($resultado) > 0) {
            while ($row = mysql_fetch_array($resultado)) {
                ...
                echo "<td class='success'><a href='../php/impresiondoc03E_1.php?documento=" . $row['DOCUMENTO'] . "&cliente=" . $row['codcli'] . "&IDB=$IDB&tipo=80&cajero=" . $row['usuario'] . "&fecha=" . $row['FECHA'] . "  ' target='imprimir.php' onclick=\"window.open(this.href, this.target, ' width=400, height=640,top=20,left=300, menubar=no');return false;\"><img src='../recursos/printer.png' width='40' height='40' alt='' /></a></td>";
                echo '</tr>';
            }
        } else {
            echo '<tr>
    ...
    case '22':
        ...
        if (mysql_num_rows($resultado) > 0) {
            while ($row = mysql_fetch_array($resultado)) {
                ...
                echo "<td class='danger'><a href='../php/impresiondoc03E_1.php?documento=" . $row['DOCUMENTO'] . "&cliente=" . $row['CEDULA'] . "&IDB=$IDB&tipo=22&cajero=" . $row['usuario'] . "&fecha=" . $row['FECHA'] . "&docm=" . $row['docmodi'] . "   ' target='imprimir.php' onclick=\"window.open(this.href, this.target, ' width=400, height=640,top=20,left=300, menubar=no');return false;\"><img src='../recursos/printer.png' width='20' height='20' alt='' /></a></td>";
                echo '</tr>';
            }
        } else {
            echo '<tr>
