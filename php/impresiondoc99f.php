    ...
    case '80':
        ...
        if (mysql_num_rows($resultado) > 0) {
            while ($row = mysql_fetch_array($resultado)) {
                ...
                $sumvntbta    = $sumvntbta + $row['VENTBTA'];
                $sumdescuento = $sumdescuento + $row['DESCUENTO'];
                $sumvntnta    = $sumvntnta + $row['VENTANET'];
                $sumiva       = $sumiva + $row['IVA'];
                $sumtotal     = $sumtotal + $row['TOTAL'];
                echo '<tr>
                    <td class="success" aling="center"><b>' . $row['TIPO'] . '</b></td>
            ...
            echo '<tr>
                  <th colspan="5">Total de Documentos: ' . $contador . '</th>
                  <th>' . $sumcantidad . '</th>
                  <th>' . $sumvntbta . '</th>
                  <th>' . $sumdescuento . '</th>
                  <th>' . $sumvntnta . '</th>
